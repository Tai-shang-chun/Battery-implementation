...
conv3 = layers.Conv1D(256, 3, padding='same', activation=tf.nn.relu)(conv2)
conv4 = layers.Conv1D(256, 3, padding='same', activation=tf.nn.relu)(conv3)
...
conv5 = layers.Conv1D(32, 17, padding='same', activation=tf.nn.relu)(spatialdropout1d1)
conv6 = layers.Conv1D(32, 17, padding='same', activation=tf.nn.relu)(conv5)
...
conv9 = layers.Conv1D(32, 3, padding='same', activation=tf.nn.relu)(maxpooling1d3)
conv10 = layers.Conv1D(32, 3, padding='same', activation=tf.nn.relu)(conv9)
conv11 = layers.Conv1D(96, 9, padding='same', activation=tf.nn.relu)(conv10)
conv12 = layers.Conv1D(96, 9, padding='same', activation=tf.nn.relu)(conv11)
...
# path = './model/Full DNN/dimension_reduction2.h5'
...
np.save('./model/Full DNN/discharge_norm.npy', np.vstack([x_train_mean, x_train_std]))
...
model.save('./model/Full DNN/dimension_reduction2.h5')
