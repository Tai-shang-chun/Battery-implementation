...
conv1 = layers.Conv1D(256, 15, padding='same', activation=tf.nn.relu)(d)
conv2 = layers.Conv1D(256, 15, padding='same', activation=tf.nn.relu)(conv1)
avgpooling1d1_1 = layers.AveragePooling1D(pool_size=2, strides=2)(conv2)
conv3 = layers.Conv1D(512, 3, padding='same', activation=tf.nn.relu)(conv2)
conv4 = layers.Conv1D(512, 3, padding='same', activation=tf.nn.relu)(conv3)
avgpooling1d1_2 = layers.AveragePooling1D(pool_size=2, strides=2)(conv4)
concat1 = layers.Concatenate(axis=2)([avgpooling1d1_1, avgpooling1d1_2])
avgpooling1d2 = layers.AveragePooling1D(pool_size=2, strides=2)(concat1)
spatialdropout1d1 = layers.SpatialDropout1D(0.16)(avgpooling1d2)
conv5 = layers.Conv1D(32, 3, padding='same', activation=tf.nn.relu)(spatialdropout1d1)
conv6 = layers.Conv1D(32, 3, padding='same', activation=tf.nn.relu)(conv5)
maxpooling1d1_1 = layers.MaxPooling1D(pool_size=2, strides=2)(conv6)
conv7 = layers.Conv1D(64, 3, padding='same', activation=tf.nn.relu)(conv6)
conv8 = layers.Conv1D(64, 3, padding='same', activation=tf.nn.relu)(conv7)
maxpooling1d1_2 = layers.MaxPooling1D(pool_size=2, strides=2)(conv8)
concat2 = layers.Concatenate(axis=2)([maxpooling1d1_1, maxpooling1d1_2])
avgpooling1d3 = layers.AveragePooling1D(pool_size=2, strides=2)(concat2)
conv9 = layers.Conv1D(128, 17, padding='same', activation=tf.nn.relu)(avgpooling1d3)
conv10 = layers.Conv1D(128, 17, padding='same',activation=tf.nn.relu)(conv9)
conv11 = layers.Conv1D(256, 7, padding='same', activation=tf.nn.relu)(conv10)
conv12 = layers.Conv1D(256, 7, padding='same', activation=tf.nn.relu)(conv11)
...
# path = './model/Full DNN/dimension_reduction4.h5'
...
y_train = np.zeros(0)
for i in train_idx:
    cycles = len(charge_data[i])
    x_train = np.concatenate((x_train, charge_data[i][:100]), axis=0)
...
for i in test_idx:
    cycles = len(charge_data[i])
    x_test = np.concatenate((x_test, charge_data[i][:100]), axis=0)
...
np.save('./model/Full DNN/charge_norm.npy', np.vstack([x_train_mean, x_train_std]))
